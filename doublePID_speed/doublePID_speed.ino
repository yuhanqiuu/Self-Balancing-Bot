...
// int movement_command = 0; // 0 = stop, 1 = forward, 2 = backward, 3 = left, 4 = right
// //-------------------------------------------------------------------------
...
//       //   leftpwm = PID_speed(filteredRPMLeft);
//       //   rightpwm = PID_speed(filteredRPMLeft);
//       //   PWM_v = PID_speed(filteredRPMLeft);
    ...
    if (central)
    {
        ...
        while (central.connected())
        {
            ...
            if (customCharacteristic.written())
            {
                ...
                {
                    Serial.println("W");
                    setpoint += 0.6; // setpoint for PID for forward, 1 degree
                    stop_flag = 0;
                }
                else if (strcmp(receivedString, "S") == 0)
                {
                    Serial.println("S");
                    setpoint -= 0.6; // setpoint for PID for backward, 1 degree
                    stop_flag = 0;
                }
                else if (strcmp(receivedString, "A") == 0) // turning left 
                { // left
